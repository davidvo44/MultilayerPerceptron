...
def train(trainCl, predictCl, NeuNetwork: networkClass,
          parameter: parameterClass):

    graph = graphClass.Graph()
    if trainCl is None or predictCl is None:
    ...
    for epochI in range(parameter.epoch):
        ...
        for i in range(0, trainCl.data.shape[1], parameter.batchSize):
            ...
            dW, db = NeuNetwork.backwardPropagation(X_batch,
                                                    Y_batch, parameter)
            ...
            epoch_loss, num_batches = updateLoss(parameter, NeuNetwork,
                                                 Y_batch, epoch_loss,
                                                 num_batches)
        loss, accuracy = printEpochSituation(epoch_loss, num_batches,
                                             NeuNetwork, trainCl)
        graph.getPredictHisto(predictCl, NeuNetwork, parameter, loss, accuracy)
        parameter.learningRate *= 0.99
        if parameter.earlyStop is True:
            if earlyStop(loss, epochI) is True:
                break
    graph.drawCurve()
...
def updateLoss(parameter, NeuNetwork, Y_batch, epoch_loss, num_batches):
    ...
    if parameter.loss == "Categorical Crossentropy":
        batch_loss = -np.mean(np.sum(oneHot_Y *
                                     np.log(NeuNetwork.A[-1] + 1e-8), axis=0))
    else:
        batch_loss = -np.mean(
            np.sum(oneHot_Y *
                   np.log(NeuNetwork.A[-1] + 1e-8) + (1 - oneHot_Y) *
                   np.log(1 - NeuNetwork.A[-1] + 1e-8),
                   axis=0))
