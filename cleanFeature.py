...
def pair_plot(features, personal_info, course_name):
    ...
    for row in range(count):
        for col in range(count):
            ax = axes[row][col]
            ax.set_xlabel(utils.subjectList[col])
            ax.set_ylabel(utils.subjectList[row])
            if col > row:
                ax.set_axis_off()
                continue
            ...
            if row == col:
                for house in utils.HOUSE_ORDER:
                    house_vals = [v for i, v in enumerate(features[row])if v is not None and i < len(house_personal) and house_personal[i] == house]
                    ax.hist(house_vals, density=True, alpha=0.5, color=utils.HOUSE_COLORS[house],bins=10)
                continue
            ...
            for house in utils.HOUSE_ORDER:
                ax.scatter(all_value_x[house], all_value_y[house], color=utils.HOUSE_COLORS[house])
    plt.tight_layout()
